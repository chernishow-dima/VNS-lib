...
for el in agregate_res:
    for d in el[1]:
        ...
        if (len(randomGreedyWeights) != 0 and len(saWeights) != 0 and len(tsWeights) != 0 and len(vnsWeights) != 0):
            ...
            res_y_randomGreedy.append(sum(randomGreedyWeights) / len(randomGreedyWeights))
            res_y_sa.append(sum(saWeights) / len(saWeights))
            res_y_ts.append(sum(tsWeights) / len(tsWeights))
            res_y_vns.append(sum(vnsWeights) / len(vnsWeights))
...
plt.ylabel(r'$LP_{res}(x)/Best_{res}(x)$', fontsize=16)
...
plt.savefig(f"./images/ResImageAgregate.pdf", dpi = 300, format="pdf", bbox_inches='tight')
# plt.show()